...
class complete():
	...
	def scrapWikiInfo(link):
		...
		if(htmltext):
			...
			soup2 = BeautifulSoup( str(paragraphs[1])) 
			text = soup2.get_text()
			...
			if(checkOtherOptions):
				...
				linkForParticularAbbr="http://en.wikipedia.org"+partsOfSingleLinks[choice-1]
	...
	def searchThroughDatabase(Abbreviation):
		searchFile=open("Abbreviations.txt","r")
		expandedForm=""
	...
	def checkForHindi(word):
		searchFile=open("DictionaryofHindi.txt","r")
		flag=0
		for line in searchFile:
			searchWord=" "+word+" "
